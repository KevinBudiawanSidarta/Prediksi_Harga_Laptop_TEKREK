        ...
        if st.button("🚀 Prediksi Harga Sekarang!!", use_container_width=True):
            with st.spinner("🤖 Menganalisis Spesifikasi..."):
                ...
                <div class="success-box">
                    ...
                    <p style="font-size: 1.2rem; margin-bottom: 1rem;">Berdasarkan konfigurasi anda:</p>
                    <h1 style="font-size: 4rem; margin: 0;">€{prediction:,.0f}</h1>
                    <p style="opacity: 0.9; margin-top: 1rem;">Estimasi harga pasar</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                specs = [
                    (f"⚡ {cpu} GHz CPU", COLORS['primary']),
                    (f"💾 {ram} GB RAM", COLORS['primary']),
                ...
                price_chart = alt.Chart(chart_data).mark_bar().encode(
                    x=alt.X('Category:N', sort=['Your Prediction', 'Market Average', 'Minimum', 'Maximum']),
                    y=alt.Y('Price:Q', title='Price (€)'),
                    color=alt.Color('Category:N', scale=alt.Scale(
                        domain=['Your Prediction', 'Market Average', 'Minimum', 'Maximum'],
                        range=[COLORS['success'], COLORS['info'], '#a0aec0', '#a0aec0']
                    )),
